...
playRound :: StdGen -> [Agent] -> Int -> (History,Stats)
playRound gen agents iterations  = (history, getStatistics iterations history)
  where hist = (map (makeInteract gen iterations) (match agents))
        history = History hist agents
...
showSums :: History -> [(Agent,Int)]
showSums (History history agents ) = map (\a -> (a,sumAgent history a)) agents
...
makeAgent :: StdGen -> Int -> (Agent,Agent) -> [Agent] -> [Agent] -> (Agent,Agent)
makeAgent gen id1 (a1,a2) winners agents = (n1,n2)
    ...
    where e1 = nearest (position a1) (positions winners)
          ...
          f1  = compose d1
          f2  = compose d2
          n1  = Agent f1 (n a1++show e1) e1 d1 id1
          n2  = Agent f2 (n a2 ++ show e2) e2 d2 (id1+1)
{--
...
new gen winners base agents = flatten $ zipWith5
                                          makeAgent
                                          (infiniteGen gen)
                                          (randomRs (0,10000000) gen :: [Int])
